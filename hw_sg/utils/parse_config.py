...
from hw_sg import text_encoder as text_encoder_module
from hw_sg.base.base_text_encoder import BaseTextEncoder
from hw_sg.logger import setup_logging
from hw_sg.text_encoder import CTCCharTextEncoder
from hw_sg.utils import read_json, write_json, ROOT_PATH
...
class ConfigParser:
    ...
    def get_default_configs(cls):
        config_path = ROOT_PATH / "hw_sg" / "config.json"
        with config_path.open() as f:
            return cls(json.load(f))

    @classmethod
    def get_test_configs(cls):
        config_path = ROOT_PATH / "hw_sg" / "tests" / "config.json"
        with config_path.open() as f:
            return cls(json.load(f))
